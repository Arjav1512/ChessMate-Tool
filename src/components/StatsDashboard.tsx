...
export function StatsDashboard({ onClose }: { onClose: () => void }) {
  ...
  const loadStatistics = async () => {
    try {
      ...
      if (gamesResult.data) {
        const formattedGames = gamesResult.data.map((game: any) => ({
          id: game.id,
          white_player: game.white_player,
          black_player: game.black_player,
          result: game.result,
          uploaded_at: game.uploaded_at,
          accuracy: game.game_analysis_results?.[0]?.accuracy,
          mistakes: game.game_analysis_results?.[0]?.mistakes,
          blunders: game.game_analysis_results?.[0]?.blunders,
        }));

...
export function buildWeaknessProfile(games: WeaknessGame[]): WeaknessProfile {
  ...
  // ── 4. Phase proxy: long-game (endgame) performance ──────────────────────
  // No per-move data is stored, so true phase attribution is impossible. Game
  // length is the one stored, phase-correlated signal — surfaced as an explicit
  // LOW-confidence proxy, never as a definitive endgame claim.
  if (baseline !== null) {
    const withLen = decided.filter((g) => typeof g.analysis?.total_moves === 'number');
    const long = withLen.filter((g) => (g.analysis!.total_moves as number) > 40);
    if (long.length >= MIN_GROUP + 1) {
      const wr = score(long);
      if (wr !== null && wr < baseline - WINRATE_MARGIN) {
        weaknesses.push({
          id: 'phase:long-game',
          category: 'phase',
          title: 'Loses the thread in long games',
          detail: `In games over 40 moves you score ${pct(wr)} vs ${pct(baseline)} overall — a sign of middlegame/endgame conversion issues.`,
          severity: Math.min(90, Math.round((baseline - wr) * 180)),
          confidence: 'low', // proxy: length, not true phase attribution
          evidence: [
            `${long.length} long games (>40 moves) · ${pct(wr)} score`,
            'Proxy from game length — per-move phase analysis is a Phase-2 item',
          ],
          trend: trendOf(long, score),
          sampleSize: long.length,
        });
      }
    }
  }
